...
// AgentService uses google.protobuf.Value for MCP schemas and arguments.
export function encodeAgentValue(value) {
  if (value === null) return encodeField(1, WIRE_TYPE.VARINT, 0);
  if (typeof value === "number") {
    const bytes = new Uint8Array(8);
    new DataView(bytes.buffer).setFloat64(0, value, true);
    return concatArrays(encodeVarint((2 << 3) | WIRE_TYPE.FIXED64), bytes);
  }
  ...
  const entries = Object.entries(value || {}).map(([key, item]) => encodeField(1, WIRE_TYPE.LEN,
    concatArrays(encodeField(1, WIRE_TYPE.LEN, key), encodeField(2, WIRE_TYPE.LEN, encodeAgentValue(item)))));
...
export function decodeAgentValue(data) {
  ...
  if (value.has(2)) return new DataView(Uint8Array.from(value.get(2)[0].value).buffer).getFloat64(0, true);
  if (value.has(3)) return textDecoder.decode(value.get(3)[0].value);
  if (value.has(4)) return value.get(4)[0].value !== 0;
  if (value.has(6)) return (decodeMessage(value.get(6)[0].value).get(1) || []).map((entry) => decodeAgentValue(entry.value));
  if (value.has(5)) {
    const result = {};
    for (const field of decodeMessage(value.get(5)[0].value).get(1) || []) {
      const entry = decodeMessage(field.value);
      result[textDecoder.decode(entry.get(1)[0].value)] = decodeAgentValue(entry.get(2)[0].value);
    }
    return result;
  }
  return null;
...
export function encodeMcpToolDefinition(tool) {
  ...
  return concatArrays(
    ...
    ...(fn.description ? [encodeField(2, WIRE_TYPE.LEN, fn.description)] : []),
...
export function decodeMcpArgs(data) {
  ...
  const string = (field) => message.has(field) ? textDecoder.decode(message.get(field)[0].value) : "";
  const args = {};
  for (const field of message.get(2) || []) {
    const entry = decodeMessage(field.value);
    args[textDecoder.decode(entry.get(1)[0].value)] = decodeAgentValue(entry.get(2)[0].value);
  }
  return { name: string(1), args, toolCallId: string(3), toolName: string(5) };
}

export function encodeMcpResultSuccess({ textItems = [], imageItems = [], isError = false } = {}) {
  const content = [
    ...textItems.map((text) => encodeField(1, WIRE_TYPE.LEN, encodeField(1, WIRE_TYPE.LEN, text))),
    ...imageItems.map(({ data, mimeType }) => encodeField(2, WIRE_TYPE.LEN,
      concatArrays(encodeField(1, WIRE_TYPE.LEN, data), encodeField(2, WIRE_TYPE.LEN, mimeType)))),
  ].map((item) => encodeField(1, WIRE_TYPE.LEN, item));
  return encodeField(1, WIRE_TYPE.LEN, concatArrays(...content, encodeField(2, WIRE_TYPE.VARINT, isError ? 1 : 0)));
}

export const encodeMcpResultError = (message) => encodeField(2, WIRE_TYPE.LEN, encodeField(1, WIRE_TYPE.LEN, message));
export const encodeMcpResultToolNotFound = (name) => encodeField(5, WIRE_TYPE.LEN, encodeField(1, WIRE_TYPE.LEN, name));
