...
// Check if settings has ORouter customModels
const hasORouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:ORouter"));
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      installed: true,
      settings,
      hasORouter: hasORouterConfig(settings),
...
// POST - Update ORouter customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing ORouter configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:ORouter"));
...
// DELETE - Remove ORouter customModels only (keep other settings)
export async function DELETE() {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "ORouter settings removed successfully",
    });
  } catch (error) {
