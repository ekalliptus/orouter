    ...
    <div className="flex min-w-0 flex-col gap-6">
      ...
      {!hidePeriodSelector && (
        <div className="flex w-full items-center gap-2 sm:w-auto sm:self-end">
          <div className="grid flex-1 grid-cols-5 items-center gap-1 rounded-lg border border-border-subtle bg-surface-2/70 p-1 sm:flex sm:flex-none">
            {PERIODS.map((p) => (
              <button
                key={p.value}
                onClick={() => setPeriod(p.value)}
                disabled={fetching}
                className={`rounded-md px-3 py-1 font-mono text-xs transition-colors ${period === p.value ? "bg-primary text-white dark:text-brand-900 shadow-sm" : "text-text-muted hover:bg-surface-3/60 hover:text-text-main"}`}
              >
                {p.label}
              </button>
            ))}
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="grid grid-cols-2 items-center gap-1 rounded-lg border border-border-subtle bg-surface-2/70 p-1 sm:flex">
            <button
              onClick={() => setViewMode("costs")}
              className={`px-3 py-1 rounded-md font-mono text-xs transition-colors ${viewMode === "costs" ? "bg-primary text-white dark:text-brand-900 shadow-sm" : "text-text-muted hover:text-text-main hover:bg-surface-3/60"}`}
            >
              Costs
            </button>
            <button
