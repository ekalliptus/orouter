...
const ROOT_CA_CN = "ORouter MITM Root CA";
...
async function installCertMac(sudoPassword, certPath) {
  ...
  const deleteOld = `security delete-certificate -c "ORouter MITM Root CA" /Library/Keychains/System.keychain 2>/dev/null || true`;
...
async function updateNssDatabases(certPath, action = 'add') {
  const certName = "ORouter MITM Root CA";
